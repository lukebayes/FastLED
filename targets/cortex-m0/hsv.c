 ...
 * NOTE(lbayes):
 *
 * The FastLED project is incredible and this fork SHOULD NOT be
 * used by anyone other than me.
 *
 * This fork has been created so that we can use a subset of the HSV/RGB
 * conversion utilities without bringing in the Arduino project or the rest
 * of the FastLED project.
 *
 * The sources have been moved around so that we can compile them from a
 * standard C build pipeline as well.
 *
 * PLEASE DO NOT USE THIS FORK.
 */
...
void hsv_to_rgb_rainbow(const hsv_t *hsv, rgb_t *rgb) {
  ...
  if( ! (hue & 0x80) ) {
    ...
    if( ! (hue & 0x40) ) {
      // 10X
      if( ! ( hue & 0x20) ) {
        // We're in a cyan area
        // 100
        //case 4: // A -> B
        r = 0;
        FORCE_REFERENCE(r);
        if (val >= 245 && sat >= 245) {
          // We're very close to CYAN, no adjustment necessary
          g = 255;
          b = 255;
        } else {
          //uint8_t twothirds = (third << 1);
    ...
    if( highest == r ) {
        ...
        if( b == 0) {
            ...
            if( (g-b) > b) {
                ...
                h = HUE_AQUA;
              // NOTE(lbayes): We're in CYAN (0x00ffff) and this returns a deep blue
              // 0x003cff hsv(160, 255, 255), instead of ~hsv(127, 255, 255).
              // Removing the following line makes it return hsv(128, 255, 255)
              // h += scale8( qsub8(b, 85), FIXFRAC8(8,42));
              // But that color is also not handled properly by _rainbow.
            }
        }
